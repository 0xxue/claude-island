// ═══════════════════════════════════════════
// Claude Island — Terminal + Pixel Pet + Sound
// ═══════════════════════════════════════════
...
var pillSpinner = document.getElementById('pill-spinner');
...
function updatePetState(eventType, toolName) {
  ...
  petSpeechEl.style.animation = 'speechIn 0.3s var(--ease)';
...
dragHandle.addEventListener('mousedown', function(e) {
  ...
  island.style.transition = 'border-radius 0.4s, width 0.4s, background 0.4s, border-color 0.4s, box-shadow 0.4s';
});
...
var mockMessages = [
  { agent: 'Claude', text: '<span class="dim">asked to modify</span> <span class="file">api.js</span>', actionType: 'modify', eventType: 'permission', tool: 'Edit', code: { file: 'api.js', tag: 'diff', lines: [{ type: 'del', text: '- res.header("Access-Control-Allow-Origin", "*");' }, { type: 'add', text: '+ res.header("Access-Control-Allow-Origin", "https://app.example.com");' }] }, needsAction: true },
  { agent: 'Gemini', text: '<span class="dim">indexing complete.</span> <span class="green">queries 40% faster</span>', actionType: 'info', eventType: 'tool_done', tool: 'Bash', code: null, needsAction: false },
  { agent: 'Codex', text: '<span class="dim">execute</span> <span class="file">db:migrate --force</span>', actionType: 'execute', eventType: 'permission', tool: 'Bash', code: { file: 'terminal', tag: 'bash', lines: [{ type: 'normal', text: 'npm run db:migrate -- --force' }] }, needsAction: true },
  { agent: 'Claude', text: '<span class="dim">optimizing</span> <span class="file">webpack.config.js</span>', actionType: 'write', eventType: 'permission', tool: 'Write', code: { file: 'webpack.config.js', tag: '', lines: [{ type: 'normal', text: '  optimization: {' }, { type: 'add', text: '+   minimize: true,' }, { type: 'normal', text: '  }' }] }, needsAction: true },
  { agent: 'Cursor', text: '<span class="dim">auto-completed 12 functions in</span> <span class="file">Dashboard.tsx</span>', actionType: 'info', eventType: 'tool_done', tool: 'Edit', code: null, needsAction: false },
];

function updatePillStatus() {
  if (pendingAgents.size === 0) {
    pillText.innerHTML = 'idle';
    pillSpinner.style.display = 'none';
    updatePetState('idle');
  } else {
    pillText.innerHTML = '<span class="warn">' + notifCount + ' pending</span> · ' + Array.from(pendingAgents).join(', ');
    pillSpinner.style.display = 'inline';
  }
  footerStat.textContent = pendingAgents.size + ' agents';
}
...
function createNotification(data) {
  ...
  var h = '<div class="notif-header"><span class="prompt">❯</span> <span class="agent-badge"><span class="agent-pet-icon"><div class="pixel-pet ' + agentPet + ' tiny"><div class="sprite"></div></div></span>' + data.agent.toLowerCase() + '</span>';
  if (data.actionType) h += '<span class="action-badge ' + data.actionType + '">' + data.actionType + '</span>';
  h += '<button class="btn-jump jump-btn">Jump ↗</button>';
  h += '</div><div class="notif-body">' + data.text + '</div>';
  if (data.code) {
    h += '<div class="notif-code">';
    data.code.lines.forEach(function(l) { h += '<div class="code-line ' + l.type + '">' + l.text + '</div>'; });
    h += '</div>';
  }
  if (data.needsAction) {
    h += '<div class="notif-actions"><button class="btn btn-n action-btn">[N] Deny</button><button class="btn btn-y action-btn">[Y] Allow</button><button class="btn btn-a action-btn">[A] All</button><button class="btn btn-j action-btn">[B] Bypass</button></div>';
    notifCount++;
  }
  card.innerHTML = h;
