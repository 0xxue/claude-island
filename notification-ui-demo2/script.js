// ═══════════════════════════════════════════
// Claude Island — Black Gold + Pixel Pet + Sound
// ═══════════════════════════════════════════
...
var HEADER_H = 44;
var MAX_HEIGHT = 520;
var AGENT_COLORS = { Claude: 'var(--gold)', Codex: '#3B82F6', Gemini: 'var(--green)', Cursor: '#A78BFA' };
...
dragHandle.addEventListener('mousedown', function(e) {
  ...
  island.style.transition = 'border-radius 0.5s, width 0.5s, background 0.5s, border-color 0.5s, box-shadow 0.5s';
});
...
var mockMessages = [
  ...
  { agent: 'Claude', text: 'Optimizing webpack config.<br><span class="user-msg">You: Make bundle smaller</span>', actionType: 'Write', eventType: 'permission', tool: 'Write', code: { file: 'webpack.config.js', tag: '', lines: [{ type: 'normal', text: '  optimization: {' }, { type: 'add', text: '+   minimize: true,' }, { type: 'normal', text: '  }' }] }, needsAction: true },
...
function createNotification(data) {
  ...
  if (data.actionType) h += '<span class="action-badge">' + data.actionType + '</span>';
